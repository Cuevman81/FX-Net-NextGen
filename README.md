...
- **Storm Prediction Center (SPC)**: Day 1–3 Convective Outlooks (categorical), **Day 4–8 Severe Outlook**, Day 1–2 probabilistic Tornado/Wind/Hail with significant-severe hatching, **Fire Weather Outlooks Day 1–8**, Mesoscale Discussions (MCDs), Local Storm Reports (LSRs), and an **SPC Mesoanalysis viewer** — the full SPC catalog (~150 parameters across 11 categories: surface, upper-air analyses from 925–300 mb with frontogenesis and jet dynamics, thermodynamics, wind shear, composite indices, multi-parameter fields, heavy rain, winter weather, and fire weather) over 11 selectable sectors. Splits into **1, 2 or 4 panes**, each with its own parameter, all locked to the same sector and valid time so a comparison is of the fields rather than the clock; layout, parameters and sector persist. Panes are sized by whichever axis runs out first so none is ever clipped, and in a multi-pane layout clicking a chart enlarges it (click again to return) — on a 1280×800 screen that is 412 px → 864 px.
